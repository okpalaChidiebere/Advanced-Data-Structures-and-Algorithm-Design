...
void NaiveBayesPreprocessor(string[16][6], datum[16][6], int[16][6][4], float[16][6][4], int&, int&);
void NaiveBayesLearner(string[16][6], datum[16][6], int[16][6][4], float[16][6][4], int&, int&);
string NaiveBayesClassifier(datum[16][6], float[16][6][4], string[16], int);
void BuildTables(string[16][6], datum[16][6], string, int, int);
int UniqueValues(datum[16][6], int);
int XRefD(string[16][6], datum[16][6], int, int);
int XRefU(datum[16][6], string[16], int);
...
int main()
{
	...
	ifstream ins;
	ins.open("computer-unlabeled.txt");
...
void NaiveBayesPreprocessor(string D[16][6], datum A[16][6], int C[16][6][4], float P[16][6][4], int& t, int& n)
{
	//open test file
	ifstream infile;
	infile.open("computer-labeled.txt");
	int i, j, k;
	char x = ' ';
	string hold;
...
void NaiveBayesLearner(string D[16][6], datum A[16][6], int C[16][6][4], float P[16][6][4], int& t, int& n)
{
	float thold = (float)t;        //   <--------------------- traviS added float type casting to t
	int i, j, k;
...
the main function*/
string NaiveBayesClassifier(datum A[16][6], float P[16][6][4], string U[16], int n)// dont forget 4 on the double
{
	double p = 0.0;
	double pTemp = 0.0;
	string c;
...
void BuildTables(string D[16][6], datum A[16][6], string hold, int i, int j)
{
...
int UniqueValues(datum A[16][6], int j)
{
...
int XRefD(string D[16][6], datum A[16][6], int m, int j)
{
...
int XRefU(datum A[16][6], string U[16], int j)
{
	int i;
	//cycle through the array and return the position where the array values match
	for (i = 1; i < UniqueValues(A, j) + 1; i++)
